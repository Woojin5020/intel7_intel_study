...
def main(duration_sec):
    ...
    while time.time() - start_time < duration_sec:
        #lr = round(random.uniform(0.00001, 0.05), 5)
        lr = round(random.uniform(0.04, 0.058), 5)
        batch_size = random.choice([32, 64, 128])

        run_training(lr, batch_size)

        # dnn-train-c01.py가 val_acc를 all_dnn_results.txt에 저장함
        try:
